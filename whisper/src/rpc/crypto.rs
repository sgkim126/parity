			...
			EncryptionInner::AES(key, nonce, encode) => {
				match encode {
					AesEncode::AppendedNonce => {
						let mut buf = Builder::aes_256_gcm(&*key, &nonce).encrypt(plain.to_vec()).ok()?;
						buf.extend(&nonce[..]);
						Some(buf)
					}
					AesEncode::OnTopics(topics) => {
						...
						let off = buf.len();
						buf.extend(plain);
						let ciphertext = Builder::aes_256_gcm(&*key, &nonce).offset(off).encrypt(buf).ok()?;
						Some(ciphertext)
					}
				}
			}
			...
			DecryptionInner::AES(extract) => {
				match extract {
					AesExtract::AppendedNonce(key) => {
						...
						Builder::aes_256_gcm(&*key, &nonce).decrypt(Vec::from(&ciphertext[..nonce_offset])).ok()
					}
					AesExtract::OnTopics(num_topics, known_index, known_topic) => {
						...
						Builder::aes_256_gcm(&*key, &BROADCAST_IV).decrypt(Vec::from(&ciphertext[offset..])).ok()
					}
				}
			}
