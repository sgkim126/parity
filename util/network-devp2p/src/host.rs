...
impl IoHandler<NetworkIoMessage> for Host {
	...
	fn register_stream(&self, stream: StreamToken, reg: Token, event_loop: &mut EventLoop<IoManager<NetworkIoMessage>>) {
		match stream {
			...
			TCP_ACCEPT => event_loop.register(&*self.tcp_listener.lock(), reg, Ready::all(), PollOpt::edge()).expect("Error registering stream"),
	...
	fn update_stream(&self, stream: StreamToken, reg: Token, event_loop: &mut EventLoop<IoManager<NetworkIoMessage>>) {
		match stream {
			...
			TCP_ACCEPT => event_loop.reregister(&*self.tcp_listener.lock(), reg, Ready::all(), PollOpt::edge()).expect("Error reregistering stream"),
